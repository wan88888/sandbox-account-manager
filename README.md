...
- **回读校验**：创建时每个字段填完回读一次，React 丢字符时自动重填；点 Create 后除确认弹窗关闭外，还会在列表里确认该邮箱已出现；删除后回读确认行已消失；
- **总数核对**：跑完点一次 **Sandbox** 标签刷新列表，比对账号总数的净增/净减是否与判定成功数一致，不一致会告警并以非零退出码结束；
- **失败隔离**：创建时单个账号失败会截图、关弹窗（必要时刷新页面）再继续，不拖垮后面的账号；
- **飞书通知**：创建完成后直接把成功账号及密码，以及「删除 / 跳过 / 失败 + 失败原因分组 + 处理建议」推到内部群；
- **可选落盘**：默认不创建结果 CSV；如需留档，可配置 `OUTPUT_CSV`。
...
创建完想立刻删掉这批，继续使用本次创建时的明细表即可：

```bash
npm start -- --delete --csv ./data/accounts.csv
```
...
| `--output-with-passwords` | 落盘 CSV 写入明文密码（默认不写）                    |
| `-h`, `--help`            | 查看用法                                             |
...
| `OUTPUT_CSV`                                          | 可选的创建结果落盘路径，默认留空、不创建 CSV                         |
| `OUTPUT_CSV_WITH_PASSWORDS`                           | 落盘是否含明文密码，默认 `false`（也可用 `--output-with-passwords`） |
